...
class App extends Component {
  ...
  handleRent = async (item) => {
    await fetch('/rent', {
      ...
      body: JSON.stringify({ catalog_id: item.id, price: item.price })
    });
    this.refreshData();
  }
  ...
  refreshData = async () => {
    ...
    this.setState({
      rental: { data: rentals, loaded: true },
      catalog: {
  ...
  handleScroll = () => {
    this.setState({ fixHeader: this.appRef.current.scrollTop > 20 });
  }
...
const Carousel = ({ movies, onRent, onReturn }) => {
  const n = movies.length;
  // Triple the array so we can loop in both directions seamlessly
  const items = React.useMemo(() => [...movies, ...movies, ...movies], [movies]);

  // Start in the middle copy so both directions are available immediately
  const [offset, setOffset] = React.useState(n);
  ...
  const slide = (dir) => {
    ...
    busy.current = true;
    setAnimated(true);
    setOffset(o => o + dir * PAGE_SIZE);
  };

  const handleTransitionEnd = () => {
    busy.current = false;
    // If we've scrolled into a clone zone, silently jump to the real copy
    setOffset(o => {
      const tooFarRight = o >= n * 2;
      const tooFarLeft = o <= 0;
      if (tooFarRight || tooFarLeft) {
        setAnimated(false);
        return tooFarRight ? o - n : o + n;
      }
      return o;
    });
  };

  // Re-enable animation one frame after a silent jump (so the jump isn't visible)
  React.useEffect(() => {
    if (!animated) {
  ...
  const trackX = -(offset * (CARD_W + CARD_GAP));

  return (
    <div className="Carousel">
      <button className="Carousel__btn Carousel__btn--left" onClick={() => slide(-1)}
      ...
      <button className="Carousel__btn Carousel__btn--right" onClick={() => slide(1)}
...
class TitleList extends Component {
  render() {
    const { titles = [], loaded, title, cost = 0, onRent, onReturn } = this.props;
    ...
    return (
      <div className="TitleList">
        <div className="TitleList__header">
          <h1>{title}</h1>
          {!!cost && <Cart cost={cost} titles={titles} />}
        </div>
        {loaded ? (
          movies.length === 0 ? (
            <div className="TitleList--empty">
              {onRent ? 'No movies available.' : 'No rentals yet.'}
            </div>
          ) : onRent ? (
            <Carousel movies={movies} onRent={onRent} onReturn={onReturn} />
          ) : (
            <div className="TitleList__grid">
              {movies.map((item, i) => (
                <Item key={i} item={item} backdrop={item.backdrop_path} onReturn={onReturn} />
              ))}
            </div>
          )
        ) : <Loader />}
